...
const DEFAULT_SPECS: &[(&str, &str)] = &[
    (
        "vite.yml",
        include_str!("../../../frameworks/vite.yml"),
    ),
    (
        "nextjs.yml",
        include_str!("../../../frameworks/nextjs.yml"),
    ),
    (
        "nuxt.yml",
        include_str!("../../../frameworks/nuxt.yml"),
    ),
    (
        "astro.yml",
        include_str!("../../../frameworks/astro.yml"),
    ),
];
    ...
    match serde_json::to_string_pretty(state) {
        Ok(json) => {
            if let Err(err) = fs::write(&path, json) {
                log_error(&format!("frameworks: failed to write {}", path.display()), &err);
            }
        }
    ...
    for (filename, contents) in DEFAULT_SPECS {
        ...
        if !path.exists() {
            if let Err(err) = fs::write(&path, contents) {
                log_error(&format!("frameworks: failed to seed {}", path.display()), &err);
                continue;
            }
        ...
        let current = match fs::read_to_string(&path) {
            ...
            Err(err) => {
                log_error(
                    &format!("frameworks: could not read {} to check for upgrade", path.display()),
                    &err,
                );
                continue;
            }
        ...
        match state.hashes.get(*filename) {
            Some(last) if *last == current_hash => {
                if let Err(err) = fs::write(&path, contents) {
                    log_error(&format!("frameworks: failed to upgrade {}", path.display()), &err);
                    continue;
                }
                log(&format!("frameworks: upgraded {filename} to new bundled default"));
                state.hashes.insert((*filename).to_owned(), new_hash);
            }
            _ => log(&format!(
...
fn scan(frameworks_dir: &Path) -> Vec<(String, FileOutcome)> {
    ...
    for path in &entries {
        ...
        let raw = match fs::read_to_string(path) {
            Ok(raw) => raw,
            Err(err) => {
                results.push((filename, FileOutcome::Rejected(format!("could not read file: {err}"))));
                continue;
            }
        };
...
pub fn validate_report(frameworks_dir: &Path) -> bool {
    ...
    if results.is_empty() {
        println!("no framework spec files found in {}", frameworks_dir.display());
        return false;
    }
    ...
    fn duplicate_agent_name_first_file_wins() {
        ...
        let outcomes: Vec<_> = results
            ...
            .map(|(name, outcome)| {
                (
                    name.as_str(),
                    matches!(outcome, FileOutcome::Accepted(_)),
                )
            })
            .collect();

...
use super::spec::{FrameworkSpec, SUPPORTED_SCHEMA_VERSION, YamlFrameworkSpec};
...
pub fn validate(raw_yaml: &str) -> Result<FrameworkSpec, String> {
    ...
    let bin_re =
        Regex::new(&yaml.bin_path_pattern).map_err(|e| format!("invalid bin_path_pattern: {e}"))?;
...
"#
    ...
    fn rejects_empty_required_field() {
        ...
        assert_eq!(
            validate(&yaml).unwrap_err(),
            "empty required field: agent_name"
        );
    }
